...
export default async function AdminUserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="dashboard-page">
    ...
    {isSuperAdmin && !user.userRoles.some((item) => item.role.name === "ADMIN" || item.role.name === "SUPER_ADMIN") ? <section className="dashboard-panel-section"><h2>Hesap yönetimi</h2><AdminDeleteButton endpoint={`/api/admin/users/${user.id}`} label="Bu hesap" /></section> : null}
