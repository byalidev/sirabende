...
export default async function AdminUserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const hasAdminRole = user.userRoles.some((item) => item.role.name === "ADMIN" || item.role.name === "SUPER_ADMIN");
  ...
  return <div className="dashboard-page">
    ...
    {isSuperAdmin && !user.userRoles.some((item) => item.role.name === "SUPER_ADMIN") ? <section className="dashboard-panel-section"><h2>Yetki yönetimi</h2><UserRoleControls id={user.id} currentRole={hasAdminRole ? "ADMIN" : "USER"} />{!hasAdminRole ? <><br /><AdminDeleteButton endpoint={`/api/admin/users/${user.id}`} label="Bu hesap" /></> : null}</section> : null}
