...
export async function POST(request: Request) {
  try {
    ...
    if (!(await verifyPassword(currentPassword, user.passwordHash)) || newPassword.length < 8 || newPassword !== confirmation) return NextResponse.json({ error: "Mevcut şifre veya yeni şifre bilgileri hatalı." }, { status: 400 });
